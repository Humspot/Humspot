/**
 * AWS lambda function to approve an organizer submission.
 */
...
async function sendEmail(submitterEmail: string, reason: string) {
  const mailOptions = {
    ...
    to: `${submitterEmail}`,
    subject: 'You\'ve been approved as a Humspot organizer!',
    html: `
      <div style="font-family: Arial, sans-serif; color: #333;">
        <p>You are now a Humspot organizer and can submit events / attractions for approval!</p>
...
export const handler = async (gatewayEvent: APIGatewayEvent, context: Context): Promise<APIGatewayProxyResult> => {
  ...
  try {
    ...
    // Update the user to organizer account type
    query = `
      UPDATE Users
