    ...
    if (context.humspotUser) {
      ...
      if (!isUserBlocked) {
        ...
        if (res.success) {
          ...
          await presentToast({ message: "Something went wrong", color: "danger", duration: 2000 });
        }
        return;
      } else {
        setUserBlocked(true);
        await presentToast({ message: "User is blocked!", color: "danger", duration: 2000 });
      }
      setUser(undefined);
    }
  ...
  return (
    <IonPage ref={pageRef}>
      ...
      <IonContent scrollY={false}>
        ...
        <ProfileSegments user={user} showSubmissions={true} />
      </IonContent>
